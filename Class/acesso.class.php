...
if(empty($AcessoInstanciada)) {
	...
	class Acesso {
		...
		function atualizarSenha($senhaAtual, $novaSenha) {
			// Verifica se a senha atual e a nova senha não estão vazias
			...
			try {
				...
				$idUsuario = $_SESSION['dadosLogado']->id;
		
				// Consulta para verificar se a senha atual está correta
				$sql = "SELECT senha FROM tbl_usuarios WHERE id = :id";
				$stm = $this->pdo->prepare($sql);
				$stm->bindValue(':id', $idUsuario, PDO::PARAM_INT);
				$stm->execute();
				...
				if ($usuario && $usuario->senha === $senhaAtual) {
					...
					$sql = "UPDATE tbl_usuarios SET senha = :novaSenha WHERE id = :id";
					$stm = $this->pdo->prepare($sql);
					$stm->bindValue(':novaSenha', $novaSenha, PDO::PARAM_STR);
					$stm->bindValue(':id', $idUsuario, PDO::PARAM_INT);
					$stm->execute();
			...
			if(isset($_POST['acao']) && $_POST['acao'] == 'addUsuario') {
				...
				// } else {
					try{
						...
						$sql = "INSERT INTO tbl_usuarios (nome, email, telefone, endereco, cpf, foto,  login, senha, id_cargo, perm_cad_usuario, perm_relatorio, perm_add_usuario, perm_edit_usuario, perm_del_usuario, perm_edit_contato, perm_pag_principal_rm, perm_pag_principal_uc, sexo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";   
						$stm = $this->pdo->prepare($sql);   
						...
						$stm->bindValue(7, $login);   
						$stm->bindValue(8, $senha);   
						$stm->bindValue(9, $id_cargo);
						$stm->bindValue(10, $perm_cad_usuario);
						// $stm->bindValue(11, $perm_cad_contato);
						$stm->bindValue(11, $perm_relatorio);
						$stm->bindValue(12, $perm_add_usuario);
						$stm->bindValue(13, $perm_edit_usuario);
						$stm->bindValue(14, $perm_del_usuario);
						$stm->bindValue(15, $perm_edit_contato);
						// $stm->bindValue(16, $perm_del_contato);
						// $stm->bindValue(16, $perm_edit_contato_nf);
						$stm->bindValue(16, $perm_pag_principal_rm);
						$stm->bindValue(17, $perm_pag_principal_uc);
		...
		function editar() {
			if(isset($_POST['acao']) && $_POST['acao'] == 'editaUsuario') {
				...
				try{   
					$sql = "UPDATE tbl_usuarios SET nome=?, email=?, telefone=?, endereco=?, cpf=?, login=?, senha=?, id_cargo=?, foto=?, perm_cad_usuario=?, perm_relatorio=?, perm_add_usuario=?, perm_edit_usuario=?, perm_del_usuario=?, perm_edit_contato=?, sexo=?, perm_pag_principal_rm=?, perm_pag_principal_uc=? WHERE id=?";   
					$stm = $this->pdo->prepare($sql);   
					$stm->bindValue(1, $nome);   
					$stm->bindValue(2, $email);   
					$stm->bindValue(3, $telefone);   
					...
					$stm->bindValue(15, $perm_edit_contato);
					// $stm->bindValue(16, $perm_del_contato);
					$stm->bindValue(16, $sexo);
					// $stm->bindValue(17, $perm_edit_contato_nf);
					$stm->bindValue(17, $perm_pag_principal_rm);
					$stm->bindValue(18, $perm_pag_principal_uc);
					$stm->bindValue(19, $id);   
					$stm->execute();  
